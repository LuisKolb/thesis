...
def write_trec_format_output(
    filename: str, data: List[EvidenceRetrieverResult], tag: str = "NO_TAG_SPECIFIED"
) -> None:
    """
    ...
    if data:
        with open(filename, "w") as file:
            for rumor_id, authority_tweet_id, rank, score in data:
                ...
                file.write(line)
                i += 1
        logger.info(f"wrote {len(data)} lines to {filename}")
    else:
        logger.warn("data was empty, nothing was written to disk!")


# write verification output to file
def write_jsonlines_from_dicts(
    filename: Union[str, os.PathLike], dicts: List[Dict]
) -> None:
    with open(filename, "w") as file:
        for item in dicts:
